...
class NotificationKafkaResourceIT {
    ...
    void setup() {
        ...
        NotificationKafkaResource kafkaResource = new NotificationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/notification-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/notification-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
